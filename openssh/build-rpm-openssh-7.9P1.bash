...
wget -c https://cdn.openbsd.org/pub/OpenBSD/OpenSSH/portable/openssh-7.9p1.tar.gz.asc

# Verify the downloaded tarball (if GPG keys are available)
gpg --verify openssh-7.9p1.tar.gz.asc openssh-7.9p1.tar.gz

# Extract the tarball
tar zxvf openssh-7.9p1.tar.gz

# Copy the PAM configuration for sshd
cp /etc/pam.d/sshd openssh-7.9p1/contrib/redhat/sshd.pam
cp /etc/pam.d/sshd openssh-7.9p1/contrib/redhat/sshd.pam.old
...
sed -i '/BuildRequires: openssl-devel < 1.1/d' openssh-7.9p1/contrib/redhat/openssh.spec

# Edit the openssh.spec file
sed -i 's/%define no_gnome_askpass 0/%define no_gnome_askpass 1/g' openssh-7.9p1/contrib/redhat/openssh.spec
sed -i 's/%define no_x11_askpass 0/%define no_x11_askpass 1/g' openssh-7.9p1/contrib/redhat/openssh.spec
sed -i 's/BuildPreReq/BuildRequires/g' openssh-7.9p1/contrib/redhat/openssh.spec
sed -i 's/PreReq: initscripts >= 5.00/#PreReq: initscripts >= 5.00/g' openssh-7.9p1/contrib/redhat/openssh.spec

# Change ownership to root:root (if necessary)
chown root:root openssh-7.9p1/contrib/redhat/openssh.spec
...
rpmbuild -ba openssh-7.9p1/contrib/redhat/openssh.spec
